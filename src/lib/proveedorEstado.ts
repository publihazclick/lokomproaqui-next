...
// proveedor recien registrado queda 'incompleto' hasta subir el minimo de referencias de producto,
// puede entonces "enviar a revision" (enviar_proveedor_a_revision, migracion 063, bajado a 1 en la
// migracion 076 -- pedido explicito del usuario 2026-07-24), y solo aparece en Explorar Bodegas /
// la galeria publica de /infoSupplier una vez el admin lo aprueba.

export const MINIMO_PRODUCTOS_PROVEEDOR = 1;
...
export async function enviarProveedorARevision(profileId: string): Promise<{ ok: boolean; message?: string }> {
  ...
  if (error) {
    const msg = error.message?.includes('minimo_1_producto') || error.message?.includes('minimo_3_productos')
