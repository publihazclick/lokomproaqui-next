...
// reales (ProductoService.get({where:{or:[...]}}) SI implementa esa busqueda, a diferencia de la
// mayoria de las pantallas de Fase 5).

export default function ShopifyPendingPage() {
  ...
  const [pedidos, setPedidos] = useState<ShopifyPendingOrder[]>([]);
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: sessionData }) => {
      ...
      setDataUser(usuario);
      setPedidos(await fetchShopifyPendingOrders(usuario.id));
      setEstado('listo');
    });
  }, []);
  ...
  async function buscar(term: string, pendingId: number, itemIndex: number) {
    ...
    const res = await buscarProductoParaShopify(term);
  ...
  function todoListo(pending: ShopifyPendingOrder): boolean {
    return pending.items.every((it) => !!it.product_id);
  }

  async function confirmar(pending: ShopifyPendingOrder) {
    if (!dataUser) return;
    ...
    const res = await resolverPedidoShopify(pending, dataUser.id, pending.items);
    setGuardandoId(null);
    if (!res.success) {
    ...
    <div className="mx-auto w-full max-w-[1100px] px-3 py-6">
      <div className="rounded-xl border border-gray-100 shadow-sm">
        <div className="flex items-center gap-2 rounded-t-xl bg-[#0d6efd] px-4 py-3 text-white">
          ...
          <h4 className="text-lg font-bold">Pedidos Shopify por Revisar</h4>
        </div>
        <div className="p-5">
          {pedidos.length === 0 && <p className="py-10 text-center text-gray-500">No hay pedidos de Shopify pendientes por revisar.</p>}

          <div className="space-y-4">
            {pedidos.map((pending) => (
              <div key={pending.id} className="rounded-lg border border-gray-200 p-4">
                <h5 className="font-semibold text-gray-800">Pedido Shopify {pending.shopify_order_number || pending.shopify_order_id}</h5>
                  ...
                  <table className="w-full min-w-[700px] text-sm">
                    <thead>
                      <tr className="border-b border-gray-200 text-left text-xs font-semibold uppercase text-gray-500">
                        <th className="py-2 pr-3">Producto (Shopify)</th>
