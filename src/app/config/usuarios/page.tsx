...
import { fetchUsuariosAdmin, fetchRolesAsignables, type UsuarioAdminRow, type RolOpcion } from '@/lib/usuariosAdmin';
import { FormUsuarioModal } from '@/components/FormUsuarioModal';

// Port de UsuariosComponent (Angular, "Usuarios" -- directorio admin). Ver
// src/lib/usuariosAdmin.ts para el detalle de los bugs reales corregidos: busqueda/paginacion
// nunca implementadas en el backend (traia TODA la tabla profiles sin filtrar), y el cambio de rol
// (la funcion mas importante de esta pantalla) nunca se guardaba.
//
// Solo administrador puede entrar -- no esta linkeada en ningun menu real tampoco en el original,
// y cambiar el rol de otro usuario es una capacidad sensible.
...
export default function UsuariosPage() {
  ...
  const cargar = useCallback(async (page: number, reemplazar: boolean, search: string) => {
    ...
    setLoader(true);
    const res = await fetchUsuariosAdmin({ search, page, limit: LIMIT });
    setLoader(false);
    setUsuarios((prev) => {
  ...
  return (
    <div className="mx-auto w-full max-w-[1140px] px-3 py-6">
      <div className="rounded-t-xl bg-[#0d6efd] px-4 py-3 text-white">
        <h4 className="text-lg font-bold">Usuarios</h4>
      </div>
      <div className="rounded-b-xl border border-t-0 border-gray-100 p-4 shadow-sm">
        ...
        <div className="mt-4 overflow-x-auto">
                ...
                {usuarios.map((u) => (
                  <tr key={u.id} className="border-b border-gray-100">
                    <td className="py-2 pr-3">
                      <button onClick={() => setEditando(u.id)} className="flex items-center gap-1 rounded bg-[#0d6efd] px-2 py-1 text-xs text-white">
                        <Pencil className="h-3 w-3" /> Editar
                      </button>
                    </td>
                    <td className="py-2 pr-3">{u.nombre}</td>
                    <td className="py-2 pr-3">{u.rolNombre}</td>
          ...
          {!cargando && usuarios.length === 0 && <p className="py-10 text-center text-gray-500">No hay usuarios para mostrar.</p>}
        </div>
